    ...
    methods:{
        ...
        fillWeekday(year, mouth, today, weekday, all){
            ...
            if(mouthSE.start != 1){
                ...
                for (let i = 0; i < lT; i++) {
                    var tps = { d: lastMouthDay - i, w: (mouthSE.first - (i + 1)) % 7, y: year, m: mouth, isThisMouth: false }
                    res.unshift(tps)
                }
            }
            ...
            if(mouthSE.end != 0){
                ...
                for (let i = 0; i < sT; i++) {
                var tpe = { d: i + 1, w: (mouthSE.end + (i + 1)) % 7, y: year, m: mouth, isThisMouth: false }
                res.push(tpe)
                }
            }
            return res
        ...
        if (!this.cvcOpen){
            if (this.currentView == "mouth" && cP < -25) {
        ...
        var angle = {
            start: this.currentView == "week" ? 0 : -180,
            end: this.currentView == "week" ? -180 : 0
        }
        ...
        if (!mode){
            mode = this.currentView == "week" ? "mouth" : "week"
        }
        ...
        var calContainer = {
            opacity: [1, 0, 1],
            height: [this.cc_height_mouth, this.cc_height_week, this.cc_height_week],
            ease: "ease-in-out"
        }

        var calControl = {
            opacity: [1, 0],
            height: [this.ct_height_mouth, this.ct_height_week],
            translate: [
                [0, 0],
                [0, -30]
            ],
        ...
        if(mode == "week"){
            ...
            var currentWeekCount = this.idCurrentWeek(this.currentSelected, this.thisMouthInWeek)
            this.setData({
            currentWeekCount: currentWeekCount,
            })
